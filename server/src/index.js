...
import { createPublicClient, http, hexToBigInt, encodeAbiParameters, parseAbiParameters, getAddress } from 'viem';
...
async function checkActivation(sender) {
    try {
        const validSender = safeAddress(sender);
        if (validSender === "0x0000000000000000000000000000000000000000")
            return true; // Default to charged if invalid
...
async function getRadrsPrice() {
    ...
    try {
        ...
        if (response.data && response.data.pairs && response.data.pairs.length > 0) {
            ...
            const pair = response.data.pairs.find((p) => p.chainId === 'bsc' && p.dexId === 'pancakeswap');
            if (pair) {
...
async function calculateFeesAsync(userOp) {
    ...
    // Check Activation
    const isActivated = await checkActivation(userOp.sender);
...
const handleSponsor = async (req, res) => {
    try {
        ...
        const encodedData = encodeAbiParameters(parseAbiParameters('address, uint256, address, uint48, uint48, address, bytes'), [feeToken, feeAmountToSign, receiver, validUntil, validAfter, payer, signature]);
